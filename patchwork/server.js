...
//
// This server is identical in shape to tic-tac-toe/server.js - the HTTP
// scaffold (routing, JSON helpers, static file serving, in-memory store)
// is genuinely game-agnostic and duplicated here rather than shared, per
// the current per-folder-independent-deploy convention (CLAUDE.md). Once
// Patchwork's real rules land in engine.js and a second game confirms
// what's actually shared vs. game-specific, this scaffold is the
// candidate to extract into a real shared package.
...
server.listen(PORT, () => {
  console.log(`Patchwork (placeholder) server listening on port ${PORT}`);
});
